...
# You need to get each file's ID from the URL
model_file_ids = [
    "1kD4nGCCZqKoZmD0W60aqWFSs-7idhoUt",  # DistilBERT model file (e.g., tf_model.h5)
    "1tHwxwGsLbnCsgH8JeIIhiB_UwmyW82QN",  # DistilBERT config file (e.g., config.json)
]

# Directory where you want to store the model
text_model_path = "models/distilbert_model"

# Ensure the models directory exists
os.makedirs(text_model_path, exist_ok=True)

# Download the model and config files
st.info("Downloading DistilBERT model and config files...")
for file_id in model_file_ids:
    download_from_gdrive(file_id, os.path.join(text_model_path, f"{file_id}.h5"))  # Saving model and config files with unique IDs

# Ensure both the model and config file exist
model_file = os.path.join(text_model_path, "1kD4nGCCZqKoZmD0W60aqWFSs-7idhoUt.h5")
config_file = os.path.join(text_model_path, "1tHwxwGsLbnCsgH8JeIIhiB_UwmyW82QN.h5")

if os.path.exists(model_file) and os.path.exists(config_file):
    # Load the model using the downloaded files
    st.info("Loading the DistilBERT model...")
    text_model = TFDistilBertForSequenceClassification.from_pretrained(text_model_path)
    st.write("Model loaded successfully!")
else:
    st.error("Error: Model files or config file not found.")
...
tokenizer = DistilBertTokenizer.from_pretrained('models/image_model/DistilBERT_with_LRDecay_tokenizer_5e-5')
image_model = load_model('models/image_model/my_model_VGG16_reducelr_1e-5.keras')
text_label_encoder = joblib.load('models/text_model/text_label_encoder.joblib')

with open('models/image_model/class_indices_my_model_VGG16.json', 'r') as f:
...
if menu == "Introduction & Objective":
    ...
    st.image(
        "images/x_train_text_dataset.png", 
    ...
    with tab2:
        ...
        st.markdown("<h3 style='font-weight: bold;'>Confusion Matrix</h3>", unsafe_allow_html=True)
        st.image("images/Streamlit/image_cm_3.png", caption="Figure 14. Image classification Confusion Matrix", use_container_width=True)
